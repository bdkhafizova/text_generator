...
class Generator:
    ...
    def __init__(self, path):             
        ...
        self.delims = [u'.', u'!', u'?', u',', u':']
        self.quotes = [u'\'', u'"', u'\u2019', u'\u2018', u'\u201d', u'\u201e', u'\u201c']
    ...
    def process_word(self, word):
        ...
        final_words = list()
        if self.sentence_end and (pure_word[0][0].isupper() or len(quotes) != 0):
            final_words.append('')
        ...
        for quote in quotes:
            if -1 < word.find(quote) < word.find(pure_word[0]):
                final_words.insert(1, quote if not quote in [u'\'', u'"'] else u'\u201c')
            elif word.find(quote) > (word.find(pure_word[0]) + len(pure_word)):
                    final_words.append(quote if not quote in [u'\'', u'"'] else u'\u201d')
        return final_words
            ...
            for book in os.listdir(directory):
                with codecs.open(directory + '/' + book, encoding='utf8') as wordfile:
                    ...
                    for raw_word in wordgen:
                        final_words = self.process_word(raw_word)
                        for word in final_words:
                            self.push_to_stats(word)
                self.push_to_stats('')
                self.cache.clear()
            print self.enter
            print self.total
